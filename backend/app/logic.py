...
def _render_contract_clauses(details: dict[str, Any]) -> dict[str, Any]:
    """
    Builds the plain-text content (title, opening paragraph, numbered
    clauses, closing line, ผู้รับจ้าง signature labels) for the 10DK
    contract, in either "company" or "individual" form depending on
    details.get("contract_type") (defaults to "company" — the original
    template). Shared by both _build_contract_text_story (PDF) and
    _build_contract_text_docx (Word) so the two formats' wording can never
    ...
    if (details.get("contract_type") or "company") == "individual":
        contractor_party = (
            f"{g('contractor_signatory')} เลขที่บัตรประชาชน {g('contractor_id_number')} "
            f"อยู่บ้านเลขที่ {g('contractor_address')}"
        )
        party_b_signee = g("contractor_signatory")
        if g("contractor_signatory_2"):
            contractor_party += (
                f" และ {g('contractor_signatory_2')} เลขที่บัตรประชาชน {g('contractor_id_number_2')} "
                f"อยู่บ้านเลขที่ {g('contractor_address_2')}"
            )
            party_b_signee += f"และ{g('contractor_signatory_2')}"

        return {
            "title": "สัญญาจ้างทำเฟอร์นิเจอร์และตกแต่งภายใน",
            "subtitle": f"สำหรับ{g('property_description')}",
            "opening": (
                f"สัญญาฉบับนี้ทำขึ้นที่{g('property_description')} ระหว่าง {g('client_name')} "
                f"เลขที่บัตรประชาชน {g('client_id_number')} อยู่บ้านเลขที่ {g('client_address')} "
                f"ซึ่งต่อไปในสัญญานี้ เรียกว่า “ผู้ว่าจ้าง” ฝ่ายหนึ่ง กับ {contractor_party} "
                f"ซึ่งต่อไปนี้ในสัญญานี้เรียกว่า “ผู้รับจ้าง” อีกฝ่ายหนึ่ง "
                f"คู่สัญญาทั้งสองฝ่ายตกลงกันทำสัญญา มีข้อความดังต่อไปนี้"
            ),
            "clauses": [
                (
                    1,
                    f"ผู้ว่าจ้างตกลงให้ผู้รับจ้างทำเฟอร์นิเจอร์ ตามแบบเฟอร์นิเจอร์ในเอกสารแนบท้ายจำนวน "
                    f"{g('attachment_pages') or '___'} หน้า ซึ่งถือเป็นส่วนหนึ่งของสัญญานี้ด้วย "
                    f"เพื่อนำไปใช้ที่{g('property_description')} รวมเป็นเงินทั้งสิ้น "
                    f"{money(details.get('total_price'))} บาท "
                    f"ทั้งนี้ ราคาดังกล่าวให้รวมถึงวัสดุอุปกรณ์ ไม้ ค่าขนส่ง ค่าสี ค่าแรงงาน "
                    f"ค่าแรงจัดของประดับตกแต่งต่างๆ และค่าใช้จ่ายอื่นๆ ทั้งหมด "
                    f"ผู้รับจ้างจะต้องเป็นผู้จัดการและเสียค่าใช้จ่ายเองทั้งสิ้น",
                ),
                (
                    2,
                    "ผู้รับจ้างสัญญาว่าจะจัดหาช่างฝีมือดีพร้อมกับควบคุมการทำงาน เพื่อทำเฟอร์นิเจอร์ตามแบบใน"
                    f"เอกสารแนบท้ายที่กำหนดไว้พร้อมทั้งจนกว่างานจะเสร็จพร้อมติดตั้ง {g('property_description')}",
                ),
                (
                    3,
                    f"ผู้ว่าจ้างตกลงให้ค่าจ้างในการทำเฟอร์นิเจอร์ตามสัญญานี้ เป็นจำนวนเงิน "
                    f"{money(details.get('total_price'))} บาท โดยแบ่งเป็น 3 งวด คือ "
                    f"งวดที่ 1 จำนวน {money(details.get('installment_1_amount'))} บาท ณ วันที่ลงนามในสัญญา "
                    f"งวดที่ 2 จำนวน {money(details.get('installment_2_amount'))} บาท ณ วันส่งมอบงาน "
                    f"งวดที่ 3 จำนวน {money(details.get('installment_3_amount'))} บาท "
                    f"ภายหลังจากที่ผู้รับจ้างได้ทำการแก้ไข defect หรือความเสียหายต่างๆ "
                    f"ที่ผู้ว่าจ้างตรวจพบเรียบร้อยแล้ว โดยชำระผ่านการโอนผ่านบัญชีธนาคารชื่อ "
                    f"{g('bank_account_name')} {g('bank_name')} {g('bank_branch')} "
                    f"เลขที่บัญชี {g('bank_account_number')}",
                ),
                (
                    4,
                    f"ผู้รับจ้างสัญญาว่าจะทำงานที่ว่าจ้างให้แล้วเสร็จภายใน {g('work_duration_days') or '60'} วัน "
                    f"นับตั้งแต่วันที่ผู้ว่าจ้างลงนามในสัญญาฉบับนี้ และได้รับเงินงวดที่ 1 จำนวน "
                    f"{money(details.get('installment_1_amount'))} บาทเรียบร้อยแล้ว "
                    f"ทั้งนี้ ผู้รับจ้างจะไม่รับผิดชอบต่อความล่าช้าที่ไม่ได้เกิดขึ้นจากตัวผู้รับจ้าง",
                ),
                (
                    5,
                    f"งานที่ไม่อยู่ในความรับผิดชอบของผู้รับจ้าง ได้แก่ "
                    f"{g('excluded_work_description') or 'งานติดตั้งตู้เสื้อผ้า'} "
                    f"รวมทั้งงานแก้ไขความเสียหายต่างๆ ที่เกิดจากความผิดพลาดของการก่อสร้างและโครงการ",
                ),
                (
                    6,
                    "ผู้รับจ้างตกลงที่จะยอมรับผิดในความเสียหายที่เกิดจากการทำงานของผู้รับจ้างเอง "
                    "อีกทั้งจะต้องรับผิดชอบในความเสียหายอื่นๆ ที่เกิดแก่ทรัพย์ของผู้ว่าจ้าง หรือบุคคลอื่นใด"
                    "ในบริเวณที่ทำงานจ้างในสถานที่ของผู้ว่าจ้างโดยการกระทำของคนงาน "
                    "ช่างหรือบริวารของผู้รับจ้างอีกด้วย",
                ),
                (
                    7,
                    f"ผู้รับจ้างรับประกันผลงานเป็นระยะเวลา 1 ปี สำหรับรายการที่สั่งผลิตตามเอกสารแนบท้ายในสัญญา "
                    f"({g('warranty_reference_note') or g('attachment_pages') or '___'}) "
                    f"นับจากวันที่ได้รับเงินงวดสุดท้าย การรับประกันนี้รวมถึงค่าใช้จ่ายในการแก้ไขซ่อมแซมในกรณี"
                    f"ที่เฟอร์นิเจอร์ได้รับความเสียหายชำรุด โดยไม่รวมรอยขีดข่วนจากการใช้งานในชีวิตประจำวันหรือ"
                    f"ความเสียหายที่เกิดจากการขนส่ง ความเสียหายที่เกิดจากการใช้งานผิดวิธีหรือผิดวัตถุประสงค์"
                    f"ของเฟอร์นิเจอร์",
                ),
            ],
            "closing": (
                "สัญญานี้ได้ทำขึ้นเป็นสองฉบับ มีข้อความตรงกัน ซึ่งคู่สัญญาทั้งสองฝ่ายได้อ่านและเข้าใจข้อความดี"
                "โดยตลอดแล้ว จึงได้ลงชื่อต่อหน้าพยาน"
            ),
            "party_b_name_line": f"({party_b_signee})",
            "party_b_extra_lines": [],
        }

    # "company" (default/original) form.
    return {
        "title": "สัญญาจ้างตกแต่งภายใน",
        "subtitle": f"สำหรับ{g('property_description')}",
        "opening": (
            f"สัญญาฉบับนี้ทำขึ้นที่{g('property_description')} ระหว่าง {g('client_name')} "
        ...
        "clauses": [
            (
                ...
                f"ยกเว้นเฟอร์นิเจอร์ รายการที่ {g('excluded_item_range')} ในเอกสารแนบท้ายสัญญา หน้า "
                f"{g('excluded_item_page')} ซึ่งผู้ว่าจ้างจะต้องชำระราคาเองตามราคาที่ซื้อจริง",
            ),
            (
                2,
            ...
            (
                ...
                f"ช่วงที่ 1 ผู้รับจ้างสัญญาว่าจะทำงานในส่วนของ {g('phase_1_rooms')} ให้แล้วเสร็จภายในวันที่ "
                f"{g('phase_1_date')} หลังจากวันลงนามในสัญญาและทำการชำระเงินงวดแรกเรียบร้อยแล้ว "
                f"ช่วงที่ 2 ผู้รับจ้างสัญญาว่าจะทำงานในส่วนของ {g('phase_2_rooms')} ให้แล้วเสร็จภายในวันที่ "
                f"{g('phase_2_date')} หลังจากวันลงนามในสัญญาและทำการชำระเงินงวดแรกเรียบร้อยแล้ว "
                f"โดยผู้ว่าจ้างตกลงจะเตรียมพื้นที่หน้างานให้อยู่ในสภาพเรียบร้อยพร้อมที่ผู้รับจ้างจะสามารถทำงานได้"
                f"โดยไม่มีผู้รับเหมารายอื่น เข้าทำงานพร้อมกันในพื้นที่หน้างาน เป็นเวลาอย่างน้อย "
                f"{g('prep_area_days') or '30'} วัน "
                f"ก่อนครบกำหนดเวลาดำเนินงานดังกล่าว แต่ถ้าผู้รับจ้างทำงานไม่แล้วเสร็จตามกำหนดเวลาดังกล่าวโดยไม่ใช่"
                f"ความผิดของผู้รับจ้าง ผู้รับจ้างไม่ต้องรับผิดต่อผู้ว่าจ้าง",
            ),
            (
                6,
            ...
            (7, "หากมีหนี้เงินที่ผู้ว่าจ้างค้างชำระ ผู้ว่าจ้างตกลงเสียดอกเบี้ยให้แก่ผู้รับจ้างในอัตราร้อยละ 15 ต่อปี"),
            (
                8,
        ...
        "party_b_name_line": f"({g('contractor_signatory')})",
        "party_b_extra_lines": [g("contractor_title"), g("contractor_name")],
    }
...
def _build_contract_text_story(details: dict[str, Any], signatures: dict[str, bytes] | None = None) -> list[Any]:
    ...
    content = _render_contract_clauses(details)
    ...
    story: list[Any] = [
        Paragraph(esc(content["title"]), title_style),
        Paragraph(esc(content["subtitle"]), subtitle_style),
        Spacer(1, 4 * mm),
        Paragraph(f"วันที่ {esc(details.get('contract_date') or '')}", date_style),
        Spacer(1, 4 * mm),
        Paragraph(esc(content["opening"]), body_style),
        Spacer(1, 3 * mm),
    ]

    for number, text in content["clauses"]:
        story.append(Paragraph(f"<b>ข้อ {number}.</b> {esc(text)}", body_style))
        story.append(Spacer(1, 2 * mm))

    story.append(Paragraph(esc(content["closing"]), body_style))
...
def _build_contract_text_docx(
    ...
    styled_para(content["title"], bold=True, size=18)
    styled_para(content["subtitle"], size=15)
    styled_para(f"วันที่ {g('contract_date')}", size=15, align=WD_ALIGN_PARAGRAPH.RIGHT)
    body_para(content["opening"])
